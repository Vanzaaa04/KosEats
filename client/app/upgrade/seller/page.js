...
export default function UpgradeSellerPage() {
  ...
  const handleNextStep = (e) => {
    e.preventDefault();
    if (step === 1) {
      if (!formData.name) return setError("Nama toko wajib diisi.");
      setError("");
      setStep(2);
    } else if (step === 2) {
  ...
  const startLivenessCheck = () => {
    ...
    const sequence = ["SMILE", "LEFT"];
    let stepIndex = 0;
    let currentTask = sequence[stepIndex];
    
    setLivenessTask(currentTask);
    setLivenessMessage(currentTask === "SMILE" ? "TANTANGAN: Buka mulut / Tersenyum LEBAR 😁" : "TANTANGAN: Menoleh ke KIRI layar");
    ...
    livenessInterval.current = setInterval(async () => {
      ...
      if (currentTask === "LEFT" && ratio > 1.5) taskPassed = true;
      ...
      if (taskPassed) {
        ...
        if (stepIndex < sequence.length) {
          ...
          setLivenessTask(currentTask);
          setLivenessMessage("BAGUS! Sekarang: Menoleh ke KIRI layar");
        } else {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api"}/auth/upgrade/seller`, {
        method: "POST",
        headers: { 
      ...
      if (data.success) {
        alert("Pendaftaran Mitra Penjual Berhasil! Menunggu persetujuan Admin.");
      ...
      <div className="flex-center" style={{ padding: "8rem 2rem 4rem 2rem" }}>
        <div className="card" style={{ maxWidth: "600px", width: "100%" }}>
          <h2 style={{ textAlign: "center", marginBottom: "1.5rem", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}>
            <Store className="text-primary" size={28} /> Upgrade Jadi Penjual
          </h2>
          ...
          {step === 1 && (
            <form onSubmit={handleNextStep} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <input className="form-input" placeholder="Nama Toko / Warung" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} required />
              <textarea className="form-input" placeholder="Deskripsi Singkat (Cth: Menyediakan masakan rumahan enak dan murah)" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} />
